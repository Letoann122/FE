        ...
        <div class="card shadow-sm border-0 mb-4 rounded-4">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center rounded-top-4">
                ...
                <div class="btn-group" role="group" aria-label="Inventory range">
                    <button class="btn btn-sm" :class="range === '7d' ? 'btn-danger' : 'btn-outline-secondary'"
                        @click="setRange('7d')" :disabled="loading">
                        7 ngày
                    </button>
                    <button class="btn btn-sm" :class="range === '1m' ? 'btn-danger' : 'btn-outline-secondary'"
                        @click="setRange('1m')" :disabled="loading">
                        30 ngày
                    </button>
                    <button class="btn btn-sm" :class="range === '3m' ? 'btn-danger' : 'btn-outline-secondary'"
                        @click="setRange('3m')" :disabled="loading">
                        3 tháng
                    </button>
                    <button class="btn btn-sm" :class="range === '1y' ? 'btn-danger' : 'btn-outline-secondary'"
                        @click="setRange('1y')" :disabled="loading">
                        1 năm
                    </button>
                </div>
            </div>
            ...
            <div class="card-body p-4">
                <div class="chart-wrap chart-wrap--large">
                    ...
                    <div v-else class="chart-placeholder text-center py-5 bg-light rounded-4">
                        <p class="text-muted mb-0">
        ...
        <div class="row g-4 mb-4 align-items-stretch">
            ...
            <div class="col-lg-5">
                <div class="card shadow-sm border-0 h-100 rounded-4">
                    <!-- ✅ header có range giống hiệu suất chiến dịch -->
                    <div
                        class="card-header bg-white py-3 d-flex justify-content-between align-items-center rounded-top-4">
                        <h5 class="card-title fw-bold mb-0">
                            <i class="bi bi-pie-chart-fill me-2"></i>Thống kê Lịch hẹn (theo
                            trạng thái)
                        </h5>
                    ...
                    <div class="card-body d-flex flex-column gap-3">
                        ...
                        <div class="w-100" style="height: 260px">
                            <Doughnut v-if="hasAppointmentSummary" :data="appointmentDonutData"
                                :options="appointmentDonutOptions" />
                            <div v-else class="text-center py-4 bg-light rounded-4 border border-1 border-dashed">
                                <p class="text-muted mb-0">
                        ...
                        <div class="row g-3">
                            <div class="col-6">
                                <div class="p-3 bg-light rounded-4 text-center border border-1">
                          ...
                          <!-- ✅ 5 badge đưa ra ngoài, tách riêng -->
      <div class="d-flex flex-wrap gap-2 justify-content-center mt-3">
        <span class="badge rounded-pill bg-secondary">REQUESTED: {{ apCount.requested }}</span>
        <span class="badge rounded-pill bg-primary">APPROVED: {{ apCount.approved }}</span>
        <span class="badge rounded-pill bg-success">COMPLETED: {{ apCount.completed }}</span>
        <span class="badge rounded-pill bg-danger">REJECTED: {{ apCount.rejected }}</span>
        <span class="badge rounded-pill bg-warning text-dark">CANCELLED: {{ apCount.cancelled }}</span>
      </div>
                ...
                <div class="card shadow-sm border-0 h-100 rounded-4">
                    ...
                    <div class="card-body">
                        <div class="chart-wrap chart-wrap--mid">
                            ...
                            <div v-else class="chart-placeholder text-center py-5 bg-light rounded-4">
                                <p class="text-muted mb-0">
                                    Chưa có chiến dịch hoặc chưa có dữ liệu trong khoảng thời gian
                                    này.
                                </p>
                            </div>
                        </div>

                        <!-- ✅ Scroll list để card không bị dài hơn bên trái -->
                        <div class="mt-3 campaign-list-scroll">
                            <div v-if="campaignList.length">
                                <div v-for="c in campaignList" :key="c.id" class="mb-3 p-3 bg-light rounded-4">
                                    <div class="d-flex justify-content-between align-items-start gap-2">
                                        ...
                                        <div class="text-end">
                                            <div class="fw-bold" :class="Number(c.completion_rate) >= 80
                                                    ? 'text-success'
                                                    : 'text-warning'
                                                ">
                                                {{ Number(c.completion_rate || 0) }}%
                                            </div>
                                            <div class="text-muted small">
                                                {{ formatNumber(c.volume_ml) }} ml
                                            </div>
                                        </div>
                                    </div>

                                    <div class="progress mt-2" style="height: 8px">
                                        <div class="progress-bar" :class="Number(c.completion_rate) >= 80
                                                ? 'bg-success'
                                                : 'bg-warning'
                                            " role="progressbar" :style="{ width: Number(c.completion_rate || 0) + '%' }"></div>
                                    </div>
        ...
        <div class="card shadow-sm border-0 mb-4 rounded-4">
            <div class="card-header bg-white py-3 rounded-top-4">
                <h5 class="card-title fw-bold mb-0">
                    <i class="bi bi-exclamation-triangle-fill me-2 text-danger"></i>Báo
                    cáo Cảnh báo
                </h5>
            </div>
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div v-for="w in warnings" :key="w.key" class="col-md-3">
                        <div class="alert-box" :class="w.class">
                            ...
                            <div class="progress" style="height: 6px">
                                <div class="progress-bar" :style="{ width: w.level + '%' }"></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-4">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        ...
                        <div class="text-muted small">
                            Tính theo blood_inventory (units &gt; 0)
                        </div>
                    </div>
...
export default {
    ...
    computed: {
        ...
        appointmentDonutData() {
            ...
            return {
                labels: ["REQUESTED", "APPROVED", "REJECTED", "COMPLETED", "CANCELLED"],
                datasets: [
                    {
                        data: values,
                        backgroundColor: [
                            this.palette.gray, // requested
                            this.palette.blue, // approved
                            this.palette.out, // rejected
                            this.palette.in, // completed
                            this.palette.amber, // cancelled
                        ],
                        borderColor: "#ffffff",
                        borderWidth: 2,
                    },
        ...
        warnings() {
            ...
            arr.push({
                ...
                title: first?.blood_type?.type_name
                    ...
                    : "Thiếu máu",
                value: first ? `${first.units} đơn vị` : "—",
                desc: first
                    ? "Tồn kho đang thấp, cần bổ sung."
                    : "Không có cảnh báo thiếu máu.",
                level: first
                    ? Math.max(5, Math.min(100, (Number(first.units) / 30) * 100))
                    : 0,
                class:
            ...
            arr.push({
                key: "expiring",
                title: "Sắp hết hạn",
                value: `${Number(exp || 0)} đơn vị`,
                desc: "Tổng đơn vị sẽ hết hạn trong 3 ngày.",
                level: Number(exp || 0) > 0 ? 60 : 5,
                class: Number(exp || 0) > 0 ? "alert-box-warning" : "alert-box-good",
            });
            ...
            arr.push({
                ...
                desc: "Số lịch REQUESTED trong kỳ.",
                level: pending ? Math.min(100, Number(pending) * 10) : 5,
                class: pending && pending >= 10 ? "alert-box-warning" : "alert-box-ok",
            });

            arr.push({
                key: "total_units",
                title: "Tổng đơn vị",
                value: totalUnits === null ? "—" : `${Number(totalUnits)} đơn vị`,
                desc: "Tổng đơn vị máu thống kê trên dashboard.",
                level: totalUnits ? Math.min(100, Number(totalUnits) * 3) : 5,
                class: "alert-box-good",
            });
        ...
        inventoryByTypeRows() {
            ...
            return rows.map((x) => {
                ...
                return {
                    ...
                    type,
                    units,
                    levelLabel: (map[level] || map.full).label,
                    badgeClass: (map[level] || map.full).cls,
                };
            });
        },
    },
...
/* ✅ làm list campaign scroll để cân layout */
.campaign-list-scroll {
    max-height: 285px;
    /* bạn có thể chỉnh 260-340 tuỳ màn */
    overflow: auto;
    padding-right: 6px;
}
...
.campaign-list-scroll::-webkit-scrollbar-thumb {
    background: rgba(148, 163, 184, 0.5);
    border-radius: 10px;
}

.campaign-list-scroll::-webkit-scrollbar-track {
